...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void handleAck(String qrText) async {
    try {
      ...
      if (response.data['data'] != null) {
        ...
        showPrescriptionModal(context, prescription);
      }
    } catch (error) {
  ...
  void handleScannedData(String qrText) async {
    try {
      var state = context.read<OrderBloc>().state.orderList;
      if (state.isEmpty) {
        setState(() {
  ...
  void showPrescriptionModal(BuildContext context, Prescription prescription) {
    showDialog(
      context: context,
  ...
  Future initialData() async {
    ...
    if (userData != null) {
      ...
      await dioHelper.initializeDio(userData);
      await userDataService.loadUserData(userData);
      userDataService.updateUserBloc(context);

      await dioHelper.fetchUsers(context);
      await dioHelper.fetchOrder(context);
      await dioHelper.fetchDrugs(context);
